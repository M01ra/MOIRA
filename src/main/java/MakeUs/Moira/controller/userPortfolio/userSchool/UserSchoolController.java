...
public class UserSchoolController {
    ...
    public ListResult<SchoolInfoResponseDto> searchSchoolInfo(@ApiParam(value = "검색어", example = "서울대", required = true) @RequestParam String keyword) {
        // 권한 설정 : 유저만 접근
    ...
    public ListResult<MajorInfoResponseDto> searchMajorInfo(@ApiParam(value = "검색어", example = "국어국문", required = true) @RequestParam String keyword) {
        // 권한 설정은 : 유저만 접근
    ...
    public ListResult<UserSchoolResponseDto> addUserSchool(@RequestHeader(value = "X-AUTH-TOKEN") String token,
                                                           @RequestBody UserSchoolAddRequestDto userSchoolAddRequestDto) {
        // 권한 설정은 시큐리티에서 하자
