...
public class homeController {
    ...
    @ApiOperation(
            ...
            notes = "### 홈화면에서 알람 목록을 확인합니다.\n" +
                    ...
                    "- PROJECT의 경우 해당 알림 클릭시 alarmTargetId를 projectId로 사용하여 /project/{projectId} 팀원 모집 - 팀 모집글 상세로 리다이렉팅 시킵니다.\n" +
                    "- REVIEW의 경우 해당 알림 클릭시 alarmTargetId를 userId로 사용하여 /review/detail/{userId} 유저의 모든 리뷰 내용 조회로 리다이렉팅 시킵니다.\n" +
                    "- CHATROOM의 경우 해당 알림 클릭시 alarmTargetId를 chatRoomId로 사용하여 /chatroom/{chatRoomId} 유저와 채팅 내용 불러오기로 리다이렉팅 시킵니다.\n" +
                    "- APPLY의 경우 해당 알림 클릭시 alarmTargetId를 projectApplyId로 사용하여 /apply/{projectApplyId} 마이페이지 - 지원한 글 - 지원 목록 - 지원 내역로 리다이렉팅 시킵니다.\n" +
                    "- APPLY_ANSWER의 경우 지원자를 수락할지 거절할지 선택하여 alarmTargetId를 projectApplyId로 사용하여 /apply/{projectApplyId} 지원서의 상태를 변경을 통해 상태를 변경시킬 수 있도록 합니다.\n" +
                    "- INVITE_ANSWER의 경우 팀장의 초대를 수락할지 거절할지 선택하여 alarmTargetId를 projectApplyId로 사용하여 /apply/{projectApplyId} 지원서의 상태를 변경을 통해 상태를 변경시킬 수 있도록 합니다."
    )
