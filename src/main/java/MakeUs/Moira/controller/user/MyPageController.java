...
//        if (token == null) throw new CustomAuthorizationException("로그인이 필요한 기능");
    ...
    public CommonResult getWrittenPost(@RequestHeader(value = "X-AUTH-TOKEN", required = true) String token) {
        // 권한 설정은 시큐리티에서 하자
        Long userId = Long.parseLong(jwtTokenProvider.getUserPk(token));
        List<WrittenProjectInfoResponseDto> writtenProjectInfoResponseDtoList = myPageService.getWrittenPost(userId);

        return responseService.mappingSingleResult(writtenProjectInfoResponseDtoList, "내가 작성한 글");
    }
}
